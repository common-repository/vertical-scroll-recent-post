...
class Vsrp_Settings {
	...
	public function vsrp_admin_options() {
		...
		if ( isset($_GET['vsrp_id']) ) {
			$get_vsrp_id = intval($_GET['vsrp_id']);
		}
		else {
			unset( $get_vsrp_id );
		}
		...
		$post_vsrp_form_submit = isset($_POST['vsrp_form_submit']) ? sanitize_text_field($_POST['vsrp_form_submit']) : '';
		...
		if ( $scrollings == false ) {
			...
			$vsrp_id += 1;
			if ( isset( $post_vsrp_form_submit  ) && $post_vsrp_form_submit  == 'yes' ) {
				$vsrp_id += 1;
			}
		}
		
		if ( isset( $get_vsrp_id ) && isset( $get_action ) && $get_action == "delete" ) {
			unset( $scrollings[ $get_vsrp_id ] );
		...
		<div class="wrap">
			...
			<h2><?php _e( 'Vertical Scroll Recent Post', 'vertical-scroll-recent-post' ); ?>
				<a href="<?php echo esc_html($settings_url) . "&action=new&vsrp_id=" . esc_html($vsrp_id); ?>" class="add-new-h2"><?php _e( 'Add New', 'vertical-scroll-recent-post' ); ?></a>
			</h2>
			<?php
			if ( isset( $post_vsrp_form_submit  ) && $post_vsrp_form_submit  == 'yes' && check_admin_referer( 'vsrp_form_setting' ) ) {
				
				$vsrp_title = isset($_POST['vsrp_title']) ? sanitize_text_field($_POST['vsrp_title']) : '';
				$vsrp_dis_num_height = isset($_POST['vsrp_dis_num_height']) ? intval($_POST['vsrp_dis_num_height']) : '35';
				$vsrp_title_length = isset($_POST['vsrp_title_length']) ? intval($_POST['vsrp_title_length']) : '30';
				$vsrp_dis_num_user = isset($_POST['vsrp_dis_num_user']) ? intval($_POST['vsrp_dis_num_user']) : '5';
				$vsrp_select_num_user = isset($_POST['vsrp_select_num_user']) ? intval($_POST['vsrp_select_num_user']) : '10';
				$vsrp_select_orderby = isset($_POST['vsrp_select_orderby']) ? sanitize_text_field($_POST['vsrp_select_orderby']) : 'date';
				$vsrp_select_order = isset($_POST['vsrp_select_order']) ? sanitize_text_field($_POST['vsrp_select_order']) : 'DESC';
				$vsrp_show_category_link = isset($_POST['vsrp_show_category_link']) ? intval($_POST['vsrp_show_category_link']) : '0';
				$vrsp_show_thumb = isset($_POST['vrsp_show_thumb']) ? intval($_POST['vrsp_show_thumb']) : '0';
				$vsrp_speed = isset($_POST['vsrp_speed']) ? intval($_POST['vsrp_speed']) : '2';
				$vsrp_seconds = isset($_POST['vsrp_seconds']) ? intval($_POST['vsrp_seconds']) : '3';
				$vsrp_show_date = isset($_POST['vsrp_show_date']) ? intval($_POST['vsrp_show_date']) : '0';
				$vsrp_date_format = isset($_POST['vsrp_date_format']) ? sanitize_text_field($_POST['vsrp_date_format']) : '';
				$vsrp_exclude_categories = isset($_POST['vsrp_exclude_categories']) ? sanitize_text_field($_POST['vsrp_exclude_categories']) : '';
				$vsrp_reverse = isset($_POST['vsrp_reverse']) ? intval($_POST['vsrp_reverse']) : '0';
				$vsrp_id = isset($_POST['vsrp_id']) ? intval($_POST['vsrp_id']) : '';
				
				if ( !isset( $_POST[ 'vsrp_select_categories' ] ) ) {
					$vsrp_select_categories = array( 1 );
				} else {
					$vsrp_select_categories = $_POST['vsrp_select_categories']; // This variable is sanitized below at line 95.
				}
				
				$tmp = "";
				if ( $vsrp_exclude_categories == 1 ) {
					if ( count($vsrp_select_categories) > 0 ) {
				...
				$vsrp_select_categories = sanitize_text_field( $tmp );

				$tmp = array();
				$tmp[$vsrp_id] = array();
				$tmp[$vsrp_id]['vsrp_title'] = stripslashes( $vsrp_title );
				...
				$tmp[$vsrp_id]['vsrp_select_orderby'] = stripslashes( $vsrp_select_orderby );
				$tmp[$vsrp_id]['vsrp_select_order'] = stripslashes( $vsrp_select_order );
				...
				$tmp[$vsrp_id]['vsrp_date_format'] = stripslashes( $vsrp_date_format );
				$tmp[$vsrp_id]['vsrp_exclude_categories'] = stripslashes( $vsrp_exclude_categories );
				$tmp[$vsrp_id]['vsrp_reverse'] = $vsrp_reverse;
				$tmp[$vsrp_id]['vsrp_select_categories'] = stripslashes( $vsrp_select_categories );
				...
				<tbody>
					...
					if ( $scrollings == false ) {
						echo "<td>-</td><td>-</td><td>-</td>
								<td><a href=\"" . esc_html($settings_url) . "&action=new&vsrp_id=" . esc_html($vsrp_id) . "\">" . __( 'Add New', 'vertical-scroll-recent-post' ) . "</a></td>";
					} else {
						foreach ( $scrollings as $key => $scr ) {
							echo "<tr><td>" . esc_html($key) . "</td><td>" . esc_html($scr[ 'vsrp_title' ]) . "</td>";
							echo "<td><code onfocus=\"this.select();\">[vsrp vsrp_id='" . esc_html($key) . "' class='']</code></td>";
							echo "<td><a href=\"" . esc_html($settings_url) . "&action=edit&vsrp_id=" . esc_html($key) . "\">" . __( 'Edit', 'vertical-scroll-recent-post' ) . "</a> | ";
							echo "<a class=\"delete_link\" href=\"" . esc_html($settings_url) . "&action=delete&vsrp_id=" . esc_html($key) . "\">" . __( 'Delete', 'vertical-scroll-recent-post' ) . "</a></td>";
						}
					} ?>
					</tr>
				</tbody>
			...
			<h2 class="nav-tab-wrapper">
				...
				<a href="http://www.gopiplus.com/work/2010/07/18/vertical-scroll-recent-post/" class="nav-tab" target="_blank"><?php _e( 'Help', 'vertical-scroll-recent-post' ); ?></a>
			</h2>
			<form name="vsrp_form" method="post" action="<?php echo esc_html($settings_url); ?>">
				<input type="hidden" value="<?php echo esc_html($vsrp_id); ?>" name="vsrp_id"/>
							...
							<td>
								<fieldset>
									...
									<label for="vsrp_title">
										<input name="vsrp_title" type="text" value="<?php echo esc_html($vsrp_title); ?>" id="vsrp_title" size="30" maxlength="150" autocomplete="off" />
							...
							<td>
								<fieldset>
									...
									<label for="vsrp_dis_num_height">
										<input name="vsrp_dis_num_height" type="number" value="<?php echo esc_html($vsrp_dis_num_height); ?>" id="vsrp_dis_num_height" />
										<br /><?php _e( 'Please enter desired height for each post\'s title in widget. <br /> If any overlap in widget at front end, 
											you should change this value.', 'vertical-scroll-recent-post' ); ?> (<?php echo esc_html($txt_example); ?>: 35)
									</label>
								</fieldset>
							</td>
						</tr>
							...
							<td>
								<fieldset>
									...
									<label for="vsrp_title_length">
										<input name="vsrp_title_length" type="number" value="<?php echo esc_html($vsrp_title_length); ?>" id="vsrp_title_length" />
										<br /><?php _e( 'Please enter desired length for each post\'s title in widget.', 'vertical-scroll-recent-post' ); ?>
										(<?php echo esc_html($txt_example); ?>: 30)
									</label>
								</fieldset>
							</td>
						</tr>
										...
										<select name="vsrp_select_categories[]" multiple >
											<?php
												...
												foreach( $categories as $category ) {
													echo '<option value="' . esc_html($category->term_id) . '" ';
							...
							<td>
								<fieldset>
									...
									<label for="vsrp_dis_num_user">
										<input name="vsrp_dis_num_user" type="number" value="<?php echo esc_html($vsrp_dis_num_user); ?>" id="vsrp_dis_num_user" />
										<br /><?php _e( 'Please enter how many post titles you want to be <br />
											displayed simultaneously in the widget.', 'vertical-scroll-recent-post' ); ?> (<?php echo esc_html($txt_example); ?>: 5)
									</label>
								</fieldset>
							</td>
						</tr>
							...
							<td>
								<fieldset>
									...
									<label for="vsrp_select_num_user">
										<input name="vsrp_select_num_user" type="number" value="<?php echo esc_html($vsrp_select_num_user); ?>" id="vsrp_select_num_user" />
										<br /><?php _e( 'Please enter how many post titles you want to be <br />
											shown in widget at totall.', 'vertical-scroll-recent-post' ); ?> (<?php echo esc_html($txt_example); ?>: 10)
									</label>
								</fieldset>
							</td>
						</tr>
								...
								<fieldset>
									...
									<label for="vsrp_speed">
										<?php _e( 'Fast', 'vertical-scroll-recent-post' ); ?> 
											<input name="vsrp_speed" type="range" value="<?php echo esc_html($vsrp_speed); ?>"  id="vsrp_speed" min="0.5" max="3.5" step="0.5" /> 
							...
							<td>
								<fieldset>
									...
									<label for="vsrp_seconds">
										<input name="vsrp_seconds" type="number" value="<?php echo esc_html($vsrp_seconds); ?>" id="vsrp_seconds" min="3" />
										<br /><?php _e( 'How many seconds you want to delay between scrollings.', 'vertical-scroll-recent-post' ); ?> (<?php echo esc_html($txt_example); ?>: 5)
									</label>
								</fieldset>
							</td>
						</tr>
